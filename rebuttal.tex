...
% Keep the letter to three pages: avoid a nearly empty signature page.
\setlength{\parskip}{0.15em}
\setlength{\parindent}{0pt}
\setlist[itemize]{leftmargin=1.15em,itemsep=0.5pt,topsep=1.5pt,parsep=0pt}
\renewcommand{\baselinestretch}{0.98}\normalsize

\title{\textbf{Author Rebuttal}\\[0.35em]
\large PanAfriCon AI 2026 --- Submission 151\\[0.15em]
\normalsize Beyond Detection Parity: Localization Parity in Pedestrian Detection}
\author{Abel Adissu \quad Anwar Gashaw Yimam \quad Dawit Kindea \quad
Beakal Gizachew, PhD \quad Elefelious Getachew, PhD\\[0.25em]
\textit{Addis Ababa University}\\[0.15em]
...
\textbf{Response.}
We agree this strengthens the contribution. In plain terms, Localization Parity
asks: \emph{among pedestrians the detector already found, do the predicted boxes
fit equally well across demographic groups?} It therefore measures fairness of
box quality, not fairness of who is detected.

Section~3.2 of the camera-ready paper now situates LP next to standard criteria:
\begin{itemize}
...
\end{itemize}
We also state clearly that LP is a \emph{group-level} criterion (not individual
fairness or calibration), chosen because the safety harm of interest is
group-structured. Orthogonality to detection parity is formalized in Eq.~(1)
and the surrounding text: a detector can find groups at equal rates while still
assigning systematically looser boxes to one group.
...
\textbf{Response.}
New Section~3.3 explains why small mean IoU gaps still matter for safety.
Under a pinhole camera model, relative depth error scales with relative error
in projected box height. Concretely: for a pedestrian at $30$\,m with an
$80$\,px box height, a $\sim$2\,pp mean IoU deficit (about $2$--$4$\,px of
systematic height / center error) corresponds to roughly $0.7$--$1.5$\,m of
depth bias. At urban speeds that bias is large enough to change time-to-collision
estimates. Downstream trajectory and collision modules are typically nonlinear
in box geometry, so a group-level mean gap of this size remains safety-relevant
even when every detection clears the conventional $0.5$ IoU ``hit'' threshold.
...
result for DWBBR (Demographic-Weighted Bounding Box Regression); that
discussion is retained and sharpened in Section~5.2.
...
preserving the core contributions both reviews found valuable: a new fairness
criterion, a broad confounder-controlled audit, and transparent reporting of a
mitigation that succeeds in training but fails to generalize. We look forward
to presenting this work at PanAfriCon AI 2026.

\vspace{0.45em}
\noindent Best regards,\\[0.25em]
Abel Adissu, Anwar Gashaw Yimam, Dawit Kindea,
Beakal Gizachew, PhD, and Elefelious Getachew, PhD\\
