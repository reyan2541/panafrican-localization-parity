...
Table~\ref{tab:compare} situates prior work against the present study.

\begin{table}[!ht]
\caption{How this paper differs from prior fairness audits in pedestrian
detection. Prior work measures detection parity (miss rate or AP). We measure
continuous Localization Parity (mean IoU), with confounder control and a
training-time mitigation.}
\label{tab:compare}
\centering
\small
\setlength{\tabcolsep}{3.2pt}
\begin{tabular}{@{}p{2.45cm}p{2.05cm}p{1.85cm}p{1.45cm}p{1.85cm}@{}}
\toprule
\textbf{Study} & \textbf{What is measured} & \textbf{Coverage} & \textbf{Confounders} & \textbf{Mitigation} \\
\midrule
Wilson et~al.~\cite{wilson2019predictive}
  & AP (detection) & 1 dataset & partial & none \\
...
ICON$^2$~\cite{sudhakar2023icon2}
  & AP (detection) & 1 dataset & scale/occ. & none \\
...
The comparison in Table~\ref{tab:compare} makes the gap concrete: prior audits stop
at detection parity, usually on a narrow dataset or detector set, and none close
the loop from continuous localization measurement to a regression-level
mitigation with an out-of-sample fairness check. Filling that empty bottom-right
of the table (LP metric, four datasets, three detectors, confounder control, and
DWBBR) is exactly the contribution of this paper.
